...
select 
	...
	max(cast(datediff('2018-11-27',to_date(reg_birthday)) as int)/365) as age,
	min(datediff('2018-11-27',to_date(user_reg_tm))) as reg_days,
	min(datediff('2018-11-27',to_date(last_login_tm))) as last_login_days,
	min(datediff('2018-11-27',to_date(last_create_ord_tm))) as last_ord_days,
...
where dt = date_add('2018-11-27', -1) and length(lower(trim(user_log_acct)))>0
...
where t.dt >= date_add('2018-11-27', -365) and t.sale_ord_dt >= date_add('2018-11-27',-365) and t.sale_ord_dt <= date_add('2018-11-27',-1) 
...
where dp in ("ACTIVE",'HISTORY') and dt>=date_add('2018-11-27', -365) and length(lower(trim(pin)))>0
	and  paytime>=date_add('2018-11-27', -365) and paytime<=date_add('2018-11-27', -1)
...
where dt >= date_add('2018-11-27', -365*5) and sale_ord_dt >= date_add('2018-11-27', -365*5) and sale_ord_dt <= date_add('2018-11-27', -1) 
...
from fdm.fdm_paytrade_02_payresult_chain  
where dp in ("ACTIVE",'HISTORY') and dt>=date_add('2018-11-27', -365*5) and length(lower(trim(pin)))>0
	and  paytime>=date_add('2018-11-27', -365*5) and paytime<=date_add('2018-11-27', -1)
group by lower(trim(pin))
;
...
select 
	lower(trim(user_log_acct)) as acct,
	datediff('2018-11-27', max(dt)) as last_visit_days,
	
	--pv
	sum(case when dt>=date_add('2018-11-27', -365)
			 then pv
			 else 0
		end) as pv_1year,
	sum(case when dt>=date_add('2018-11-27', -180)
			 then pv
			 else 0
		end) as pv_6mon,
	sum(case when dt>=date_add('2018-11-27', -90)
			 then pv
			 else 0
		end) as pv_3mon,
	sum(case when dt>=date_add('2018-11-27', -30)
			 then pv
			 else 0
		end) as pv_1mon,
	sum(case when dt>=date_add('2018-11-27', -7)
			 then pv
			 else 0
		end) as pv_7day,
	sum(case when dt>=date_add('2018-11-27', -3)
			 then pv
			 else 0
		end) as pv_3day,
	sum(case when dt=date_add('2018-11-27', -1)
			 then pv
			 else 0
		end) as pv_1day,
...
where dt >= date_add('2018-11-27', -365) and dt <= date_add('2018-11-27', -1) and length(lower(trim(user_log_acct)))>0
group by lower(trim(user_log_acct))
;
...
--hive -e "set hive.cli.print.header=true;select * from tmp.djjf_sms2_train_data_pos" > /data0/ljl/sms2/pos.csv
--hive -e "set hive.cli.print.header=true;select * from tmp.djjf_sms2_train_data_neg_mall" > /data0/ljl/sms2/neg.csv
